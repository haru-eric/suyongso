<template>
    <article class='notify'>
        <h6 @click='toggle'>
            <strong>{{ $store.state.user.nickname }}</strong>님, 읽지않은 알림이 <span>{{ $store.state.user.noticeCount }}개</span> 있습니다.
        </h6>
        <ul v-if='visible'>
            <li v-for='(item, index) in notices' :key='index' @click='visible = false'>
                <nuxt-link :to='`/${item.topicId}?postId=${item.postId}`'>
                    ...
                    <div class='content'>
                        <div class='author'>
                            ...
                            <img :src='`https://suyongso.co/icon/${item.icon}`' v-if='item.icon !== ""'>
                            {{ item.author }}
                        </div>
                        <div class='regdate'>
                        ...
                        <div class='text'>
                            ...
                            <div class='sticker' v-if='item.stickerId > 0'>
                                <img :src='`https://suyongso.co/sticker/${item.stickerId}/${item.stickerSelect}`' @error='imageUrlAlt'>
                            </div>
